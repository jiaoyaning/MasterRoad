...
interface AidlInterface {
    void inTest(in String inTest);         //基本数据类型默认切必须是in，两端不能同步修改
    void outTest(out AidlBean outTest);      //自定义类型必须制定Tag，bean类不能被客户端更改，只能由服务端更改
    void inoutTest(inout AidlBean inoutTest); //服务端修改后，自动同步到客户端
     ...
     * https://zhuanlan.zhihu.com/p/143082762
     */
    oneway void onewayTest(in AidlBean onewayTest);
}
