...
import com.jyn.masterroad.ipc.aidl.AidlBean;
...
interface AidlTestInterface {
    ...
    AidlBean testFun(in AidlBean AidlBean1,in AidlBean AidlBean2);

    void setInTest(in AidlBean inTest); //基本数据类型默认切必须是in，两端不能同步修改
    void setOutTest(out AidlBean outTest); //自定义类型必须制定Tag，bean类不能被客户端更改，只能由服务端更改
    void setInOutTest(inout AidlBean inoutTest); //服务端修改后，自动同步到客户端
    AidlBean getTestBean();
     ...
     */
    oneway void onewayTest(in AidlBean AidlBean);
}
